...
const ui = {
    ...
    elements: {
        ...
        applyPresetBgBtn: document.getElementById('applyPresetBgBtn'), // Добавили кнопку
    },
    interactionCtx: null,
    isDragging: false,
    dragOffsetX: 0,
    ...
    init(app) {
        ...
        setTimeout(() => this.resizeInteractionCanvas(), 1000);
    ...
    addEventListeners() {
        this.elements.layersPanel.addEventListener('click', (e) => { if (e.target.classList.contains('delete-layer-btn')) this.app.deleteOverlay(parseFloat(e.target.dataset.id)); });
        this.elements.qrBtnDefault.addEventListener('click', () => { if (this.app.employeeData?.contact.telegram) this.app.addQrCodeOverlay(`${this.app.employeeData.contact.telegram.replace('@', '')}`); });
        ...
        // ИЗМЕНЕННЫЙ ОБРАБОТЧИК: теперь только ВЫБИРАЕТ фон
        ...
        // НОВЫЙ ОБРАБОТЧИК: ПРИМЕНЯЕТ выбранный фон
    ...
    updateSelectedPreset(selectedFilename) {
        ...
        images.forEach(img => {
            if (img.dataset.filename === selectedFilename) {
    ...
    renderLayersPanel() {
        ...
        [...overlays].reverse().forEach(overlay => {
            ...
            const nameSpan = document.createElement('span'); let name = 'Layer';
            if (overlay.type === 'text' || overlay.type === 'scrolling-text') name = overlay.data.text;
            else if (overlay.type === 'qr') name = `(QR) ${overlay.data.link}`;
            else if (overlay.type === 'image') name = overlay.group === 'logo' ? `(Лого)` : `(Изображение)`;
            if (overlay.group === 'privacy') name = `(Приватность) ${name}`;
            nameSpan.textContent = name;
            const deleteBtn = document.createElement('button'); deleteBtn.className = 'delete-layer-btn'; deleteBtn.textContent = 'X'; deleteBtn.dataset.id = overlay.id;
            item.appendChild(nameSpan); item.appendChild(deleteBtn); this.elements.layersPanel.appendChild(item);
        });
    },
    ...
    getOverlayBounds(overlay) {
        ...
        if (overlay.type === 'qr') return this.getQrOverlayBounds(overlay);
        if (overlay.type === 'image') return this.getImageOverlayBounds(overlay);
    ...
    handleMouseDown(e) {
        ...
        for (let i = draggableOverlays.length - 1; i >= 0; i--) {
            const overlay = draggableOverlays[i];
    ...
    handleMouseMove(e) {
        ...
        const { x, y } = this.getMousePos(e);
        const scale = this.getScale();
        let newX = (x - this.dragOffsetX) / scale.x, newY = (y - this.dragOffsetY) / scale.y;
        const overlay = videoProcessor.getOverlayById(this.app.selectedOverlayId);
        if (!overlay) return;
        const bounds = this.getOverlayBounds(overlay), videoWidth = 1280, videoHeight = 720;
        if (newX < 0) newX = 0; else if (newX + bounds.w > videoWidth) newX = videoWidth - bounds.w;
        if (newY < 0) newY = 0; else if (newY + bounds.h > videoHeight) newY = videoHeight - bounds.h;
        ...
        if (overlay.type === 'qr') this.updateQrEditorFromOverlay(overlay);
        if (overlay.type === 'image') this.updateImageEditorFromOverlay(overlay);
    },
    ...
    getQrOverlayBounds(overlay) { return { x: overlay.data.x - 5, y: overlay.data.y - 5, w: overlay.data.width + 10, h: overlay.data.height + 10 }; },
    getImageOverlayBounds(overlay) { return { x: overlay.data.x, y: overlay.data.y, w: overlay.data.width, h: overlay.data.height }; },
    getTextOverlayBounds(overlay) {
        ...
        return { x: overlay.data.x, y: overlay.data.y, w: maxWidth + padding * 2, h: (lines.length * overlay.data.fontSize) + padding * 2 };
    },
    ...
    updateSelectedTextOverlayFromEditor() {
        ...
        const newData = {
            fontSize: parseInt(this.elements.fontSizeInput.value, 10), textColor: this.elements.textColorInput.value,
            hasBackground: this.elements.hasBackgroundCheckbox.checked, backgroundColor: this.elements.bgColorInput.value,
            backgroundOpacity: parseFloat(this.elements.bgOpacitySlider.value), x: parseFloat(this.elements.posXSlider.value), y: parseFloat(this.elements.posYSlider.value),
        };
...
ui.handleMouseUp = function() { this.isDragging = false; this.elements.interactionCanvas.style.cursor = 'grab'; };
ui.getMousePos = function(e) { const rect = this.elements.interactionCanvas.getBoundingClientRect(); return { x: e.clientX - rect.left, y: e.clientY - rect.top }; };
ui.getScale = function() { return { x: this.elements.interactionCanvas.width / 1280, y: this.elements.interactionCanvas.height / 720 }; };
ui.getTextOverlayBounds = function(overlay) {
    const tempCtx = document.createElement('canvas').getContext('2d');
    tempCtx.font = `${overlay.data.fontSize}px ${overlay.data.fontFamily || 'Arial'}`;
    const lines = overlay.data.text.split('\n');
    let maxWidth = 0;
...
ui.showTextEditor = function(overlay) { this.elements.textEditor.style.display = 'block'; this.updateEditorFromOverlay(overlay); };
ui.hideTextEditor = function() { this.elements.textEditor.style.display = 'none'; };
ui.updateEditorFromOverlay = function(overlay) {
    if (!overlay) return;
    ...
    const maxX = 1280 - bounds.w; // чтобы текст не выходил за правый край
    const maxY = 720 - bounds.h;

    this.elements.posXSlider.max = maxX;
    this.elements.posYSlider.max = maxY;

    this.elements.posXSlider.value = overlay.data.x;
    this.elements.posYSlider.value = overlay.data.y;
};

ui.updateSelectedOverlayFromEditor = function() {
    if (!this.app.selectedOverlayId) return;
    const newData = {
...
ui.alignSelectedOverlay = function(corner) {
    if (!this.app.selectedOverlayId) return;
    const overlay = videoProcessor.getOverlayById(this.app.selectedOverlayId);
    const bounds = this.getTextOverlayBounds(overlay);
    const margin = 20; let newX, newY;
    switch (corner) {
        case 'tl': newX = margin; newY = margin; break;
        case 'tr': newX = 1280 - bounds.w - margin; newY = margin; break;
        case 'bl': newX = margin; newY = 720 - bounds.h - margin; break;
    ...
    videoProcessor.updateOverlay(this.app.selectedOverlayId, { x: newX, y: newY });
    this.updateEditorFromOverlay(videoProcessor.getOverlayById(this.app.selectedOverlayId));
};
ui.redrawInteractionLayer = function() {
    this.interactionCtx.clearRect(0, 0, this.elements.interactionCanvas.width, this.elements.interactionCanvas.height);
    const scale = this.getScale();
    const textOverlays = videoProcessor.state.overlays.filter(o => o.type === 'text');
